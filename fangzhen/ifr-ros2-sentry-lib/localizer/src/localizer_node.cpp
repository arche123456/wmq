...
#include <boost/make_shared.hpp> // 确保包含头文件
...
struct NodeConfig {
  ...
  std::string local_frame = "lidar_now";
  double update_hz = 1.0;
  // Relocalization initial values
  std::string pcd_path = "/home/ifr/ros2-sentry-lib/ifr-ros2-sentry-lib/"
                         "pcd2pgm/pcd_saves/ncut.pcd";
  float x = 0.0, y = 0.0, z = 0.0;
  float yaw = 0.0, roll = 3.14159, pitch = 0.0;
};
...
public:
  LocalizerNode() : Node("localizer_node") {
    ...
    m_cloud_sub.subscribe(this, m_config.cloud_topic,
    ...
    m_timer =
        this->create_wall_timer(10ms, std::bind(&LocalizerNode::timerCB, this));
    relocCBInit();
  }
  ...
  void loadParameters() {
    ...
    RCLCPP_INFO(this->get_logger(), "LOAD FROM YAML CONFIG PATH: %s",
                config_path.c_str());
    ...
    m_config.x = config["x"].as<float>();
    m_config.y = config["y"].as<float>();
    m_config.z = config["z"].as<float>();
    m_config.yaw = config["yaw"].as<float>();
    m_config.roll = config["roll"].as<float>();
    m_config.pitch = config["pitch"].as<float>();
  ...
  void timerCB() {
    ...
    bool update_tf =
        diff.seconds() > (1.0 / m_config.update_hz) && m_state.message_received;
    ...
    if (m_state.service_received) {
      ...
      initial_guess.block<3, 3>(0, 0) =
          (m_state.last_offset_r * m_state.last_r).cast<float>();
      initial_guess.block<3, 1>(0, 3) =
          (m_state.last_offset_r * m_state.last_t + m_state.last_offset_t)
              .cast<float>();
    }
    ...
    if (result) {
      ...
      m_state.last_offset_t =
          -map_body_r * current_local_r.transpose() * current_local_t +
          map_body_t;
  ...
  void syncCB(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg,
    ...
    m_state.last_r = Eigen::Quaterniond(odom_msg->pose.pose.orientation.w,
                         ...
                         .toRotationMatrix();
    m_state.last_t =
        V3D(odom_msg->pose.pose.position.x, odom_msg->pose.pose.position.y,
  ...
  void
  relocCB(const std::shared_ptr<interface::srv::Relocalize::Request> request,
  ...
  void relocCBInit() {
    ...
    float y = m_config.y;
    float z = m_config.z;
    float yaw = m_config.yaw;
    float roll = m_config.roll;
    float pitch = m_config.pitch;

    if (!std::filesystem::exists(pcd_path)) {
      RCLCPP_WARN(this->get_logger(), "pcd file not found");
      return;
    }
    ...
    bool load_flag = m_localizer->loadMap(pcd_path);
    if (!load_flag) {
      RCLCPP_WARN(this->get_logger(), "load map failed");
      return;
    }
    {
      std::lock_guard<std::mutex>(m_state.message_mutex);
  ...
  void
  relocCheckCB(const std::shared_ptr<interface::srv::IsValid::Request> request,
